...
import { Star, ThumbsUp, CheckCircle, MessageSquare, Sparkles, Filter, X } from 'lucide-react';
...
export const ProductReviewsSection: React.FC<ProductReviewsSectionProps> = ({ product }) => {
  ...
  const [reviews, setReviews] = useState<ReviewItem[]>([
    {
      ...
      partnerReaction: '“You remembered my favorite dialogue? This is the prettiest thing anyone has ever given me.”',
      helpfulCount: 38,
    },
    {
      id: 'rev-p2',
      ...
      partnerReaction: '“No aching earlobes even after dancing non-stop to London Thumakda!”',
      helpfulCount: 29,
    },
    {
      id: 'rev-p3',
      ...
      partnerReaction: '“She showed it to all her friends in our college group chat immediately.”',
      helpfulCount: 44,
    },
    {
      id: 'rev-p4',
      ...
      partnerReaction: '“Countless compliments from aunties and bridesmaids all evening.”',
      helpfulCount: 21,
    },
    {
      id: 'rev-p5',
      ...
      partnerReaction: '“Kept the wax-sealed envelope in her keepsake memory drawer forever.”',
      helpfulCount: 16,
    },
  ]);
  ...
  const [formTag, setFormTag] = useState<'Gifting' | 'Comfort' | 'Wedding' | 'Boyfriend Lifesaver'>('Gifting');
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    ...
    const newReview: ReviewItem = {
      ...
      partnerReaction: formReaction.trim() ? `“${formReaction.trim()}”` : undefined,
      helpfulCount: 1,
    };
  ...
  const filteredReviews = activeFilter === 'all'
    ? reviews
    : reviews.filter((r) => r.tag.toLowerCase() === activeFilter.toLowerCase());

  return (
    <section id="reviews-section" className="py-16 bg-white border-b border-[#EAE1D7]">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-6 border-b border-[#EAE1D7]">
          <div>
            ...
            <p className="text-xs sm:text-sm text-[#70605A] mt-1">
              Read real reactions, tearful unboxings, and girlfriend reviews for {product.name}.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center bg-[#FAF7F2] p-6 sm:p-8 rounded-3xl border border-[#EAE1D7]">
          ...
          <div className="lg:col-span-4 text-center lg:text-left space-y-2 lg:border-r lg:border-[#EAE1D7] lg:pr-8">
            <div className="flex items-baseline justify-center lg:justify-start space-x-2">
              <span className="text-5xl font-display font-bold text-[#1C1412]">4.9</span>
              <span className="text-base text-[#8C7A75] font-semibold">/ 5.0</span>
            </div>
            ...
            <p className="text-xs font-semibold text-[#1C1412]">
              Based on {product.reviewsCount + reviews.length - 5} verified customers
            </p>
            <p className="text-[11px] text-emerald-800 font-medium">
              ✦ 98% of recipients recommended this as their favorite romantic gift
            </p>
          </div>
          ...
          <div className="lg:col-span-5 space-y-2">
            {[
              ...
              <div key={idx} className="flex items-center text-xs space-x-3">
                <span className="w-14 text-[#70605A] text-[11px] font-medium">{bar.stars}</span>
                ...
                <span className="w-8 text-right text-[11px] text-[#8C7A75] font-mono">{bar.pct}</span>
              </div>
            ))}
          </div>
          ...
          <div className="lg:col-span-3 grid grid-cols-2 gap-3 text-center">
            <div className="p-3 bg-white rounded-2xl border border-[#EAE1D7]">
              <span className="block text-xl font-bold text-[#961A38]">100%</span>
              <span className="text-[10px] text-[#6E5D57] uppercase font-semibold">Surprise Proof</span>
            </div>
            <div className="p-3 bg-white rounded-2xl border border-[#EAE1D7]">
              <span className="block text-xl font-bold text-[#961A38]">&lt;16g</span>
              <span className="text-[10px] text-[#6E5D57] uppercase font-semibold">Featherlight</span>
            </div>
            <div className="p-3 bg-white rounded-2xl border border-[#EAE1D7]">
              <span className="block text-xl font-bold text-[#961A38]">22K</span>
              <span className="text-[10px] text-[#6E5D57] uppercase font-semibold">Anti-Tarnish</span>
            </div>
            <div className="p-3 bg-white rounded-2xl border border-[#EAE1D7]">
              <span className="block text-xl font-bold text-[#961A38]">24h</span>
              <span className="text-[10px] text-[#6E5D57] uppercase font-semibold">Air Dispatch</span>
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 sm:p-8 border border-[#EAE1D7] shadow-xl relative max-h-[90vh] overflow-y-auto">
            ...
            <div className="mb-6 space-y-1">
              ...
              <p className="text-xs text-[#70605A]">
                Help other boyfriends, sweethearts, and brides pick their perfect Bollywood keepsake.
              </p>
            </div>
            ...
            {submittedMessage ? (
              <div className="py-12 text-center space-y-3">
                ...
                <p className="text-xs text-[#6E5D57]">
                  Your review has been published with verified purchase credentials.
                </p>
              </div>
            ) : (
              <form onSubmit={handleFormSubmit} className="space-y-4">
                  ...
                  <div className="flex items-center space-x-1">
                    {[1, 2, 3, 4, 5].map((star) => (
                      ...
                      >
                        <Star
                          className={`w-6 h-6 ${
                            star <= formRating ? 'fill-current' : 'text-[#DECFC2]'
                          }`}
                        />
                      </button>
                    ))}
                ...
                <div>
                  ...
                  >
                    ...
                    <option value="Comfort">🪶 Featherlight Weight & Comfort</option>
                    <option value="Wedding">👑 Wedding & Sangeet</option>
                    <option value="Boyfriend Lifesaver">🔥 Boyfriend Lifesaver</option>
                  </select>
                </div>
